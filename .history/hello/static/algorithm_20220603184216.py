...
def MODEL(path_open,path_save,name_file_open="data.csv", n=20, k=3, Name_Model="GaussianNB"):
    Model = Name_Model()
    ...
    path_save_parent = path_save + \
        f'algorithm/{Name_Model}/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/{Name_Model}/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/{Name_Model}/{str_date}')
    ...
    fs = open(path_stock+f'bieudo/{Name_Model}.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
