# LogisticRegression
import math
import os
import csv
from matplotlib import pyplot as plt
import numpy as np
from numpy import random
import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.metrics import classification_report
from sklearn.metrics import accuracy_score
from sklearn.metrics import confusion_matrix
from scipy import stats
import pylab as pl
import shutil
from sklearn.naive_bayes import GaussianNB
from sklearn.tree import DecisionTreeClassifier
from sklearn.ensemble import RandomForestClassifier
from sklearn.neighbors import KNeighborsClassifier
from sklearn.naive_bayes import BernoulliNB
from sklearn.tree import ExtraTreeClassifier
from sklearn.ensemble import BaggingClassifier
from sklearn.ensemble import AdaBoostClassifier
from sklearn.neural_network import MLPClassifier
from sklearn.discriminant_analysis import LinearDiscriminantAnalysis



from datetime import datetime



path_stock = os.path.dirname(os.path.abspath("my_app"))+os.sep


def Train_batch(batch):
        arr = []
        with open(path_stock+"data/data.csv") as f:
            arr = f.readlines()
            arr = np.array(arr)
            random.shuffle(arr)
            arr = np.array_split(arr, batch)
        if os.path.exists(path_stock+"data/tmp"):
            shutil.rmtree(path_stock+"data/tmp")
        os.makedirs(path_stock+"data/tmp")
        for i in range(0, len(arr)):
            with open(
                path_stock+f"/data/tmp/{i+1}.csv", "w") as f: f.writelines(arr[i])

def GaussianNB_func():
    Model = GaussianNB()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/GaussianNB/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/GaussianNB/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/GaussianNB/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/GaussianNB.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("GaussianNB" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/GaussianNB.png")
    plt.show()

def DecisionTreeClassifier_func():
    Model = DecisionTreeClassifier()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/DecisionTreeClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/DecisionTreeClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/DecisionTreeClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/DecisionTreeClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("DecisionTreeClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/DecisionTreeClassifier.png")
    plt.show()

def KNeighborsClassifier_func():
    Model = KNeighborsClassifier(n_neighbors=8)
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/KNeighborsClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/KNeighborsClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/KNeighborsClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/KNeighborsClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("KNeighborsClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/KNeighborsClassifier.png")
    plt.show()

def BernoulliNB_func():
    Model = BernoulliNB()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/BernoulliNB/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/Test/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/BernoulliNB/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/BernoulliNB.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("BernoulliNB" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/BernoulliNB.png")
    plt.show()

def ExtraTreeClassifier_func():
    Model = ExtraTreeClassifier()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/ExtraTreeClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/ExtraTreeClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/ExtraTreeClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/ExtraTreeClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("ExtraTreeClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/ExtraTreeClassifier.png")
    plt.show()

def BaggingClassifier_func():
    Model = BaggingClassifier()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/BaggingClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/BaggingClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/BaggingClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/BaggingClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("BaggingClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/BaggingClassifier.png")
    plt.show()

def AdaBoostClassifier_func():
    Model = AdaBoostClassifier()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/AdaBoostClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/AdaBoostClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/AdaBoostClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/AdaBoostClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("AdaBoostClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/AdaBoostClassifier.png")
    plt.show()

def MLPClassifier_func():
    Model = MLPClassifier()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/MLPClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/MLPClassifier/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/MLPClassifier/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/MLPClassifier.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("MLPClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/MLPClassifier.png")
    plt.show()

def LinearDiscriminantAnalysis_func():
    Model = LinearDiscriminantAnalysis()
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")

    path_save_parent = path_stock + \
        f'algorithm/LinearDiscriminantAnalysis/{str_date}'+os.path.sep
    if(os.path.exists(path_stock + f'algorithm/LinearDiscriminantAnalysis/{str_date}') == False):
        os.makedirs(path_stock +
                    f'algorithm/LinearDiscriminantAnalysis/{str_date}')

    x = data_train[:, 1:11]
    y = data_train[:, 11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo') == False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/Test.csv', "a")
    for i in range(n):
        trainx = x[begin:end]
        trainy = y[begin:end]
        # model = func.fit(trainx, trainy)
        Model.fit(trainx, trainy)

        y_pred = Model.predict(x_test)
        accuracy = accuracy_score(y_pred, y_test)
        begin += int(len(x_train) / n)
        end += int(len(x_train) / n)
        arr.append(accuracy)
        x_ve.append(i)
        fs.write(f"{i} {accuracy}\n")
  

    plt.plot(x_ve, arr)
    plt.title("LinearDiscriminantAnalysis" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/LinearDiscriminantAnalysis.png")
    plt.show()

def RandomForestClassifier_func():
    Model = RandomForestClassifier(max_depth=2)
    data_train = pd.read_csv(path_stock+"data/data.csv").values
    now = datetime.now()

    # convert to string
    str_date = now.strftime("%Y-%m-%d_%H-%M-%S")
    
    path_save_parent = path_stock + \
        f'algorithm/RandomForestClassifier/{str_date}'+os.path.sep
    if(os.path.exists(path_stock+ f'algorithm/RandomForestClassifier/{str_date}')==False):
            os.makedirs(path_stock + f'algorithm/RandomForestClassifier/{str_date}')



    x = data_train[:, 1:11]
    y = data_train[:,11]
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.3)
    with open(f"{path_save_parent}/data_train_label.csv", "a") as f:
        f.write(str(y_train))
    with open(f"{path_save_parent}/data_train_value.csv", "a") as f:
        f.write(str(x_train))
    with open(f"{path_save_parent}/data_test_label.csv", "a") as f:
        f.write(str(y_test))
    with open(f"{path_save_parent}/data_test_value.csv", "a") as f:
        f.write(str(x_test))

    begin = 0
    # chia thành 10
    n = 20
    # bước nhảy k=
    k = 3
    end = int((len(x) * k) / n)
    arr = []
    x_ve = []
    if(os.path.exists(path_stock+f'bieudo')==False):
        os.makedirs(path_stock+f'bieudo')
    fs = open(path_stock+f'bieudo/RandomForestClassifier.csv', "a")
    for i in range(n):
                        trainx = x[begin:end]
                        trainy = y[begin:end]
                        # model = func.fit(trainx, trainy)
                        Model.fit(trainx, trainy)

                        y_pred = Model.predict(x_test)
                        accuracy = accuracy_score(y_pred, y_test)
                        begin += int(len(x_train) / n)
                        end += int(len(x_train) / n)
                        arr.append(accuracy)
                        x_ve.append(i)
                        fs.write(f"{i} {accuracy}\n")
                      
    plt.plot(x_ve, arr)
    plt.title("RandomForestClassifier" + str_date)
    os.makedirs(path_save_parent+'images')
    plt.savefig(path_save_parent+"images/Test.png")
    plt.show()



# os.shutil.rmtree(path_stock+"tmp")
# os.shutil.rmtree(path_stock+"bieudo")
# os.shutil.rmtree(path_stock+"algorithm")



Train_batch(10)
GaussianNB_func()
DecisionTreeClassifier_func()
KNeighborsClassifier_func()
BernoulliNB_func()
ExtraTreeClassifier_func()
BaggingClassifier_func()
AdaBoostClassifier_func()
MLPClassifier_func()
LinearDiscriminantAnalysis_func()
RandomForestClassifier_func()