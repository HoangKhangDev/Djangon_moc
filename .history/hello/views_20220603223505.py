...
from hello.static.algorithm import Select_Model 
...
def process_test(request):
    if request.method == 'POST':
        ...
        array_nu, array_gamma, array_acc, x_ve = Select_Model(
            settings.MEDIA_ROOT+uploaded_file_url, settings.MEDIA_ROOT, request.POST['number-cut'], request.POST['number-cut'], request.POST['select'])
        # plt.figure(figsize=(7,5))
        plt.plot(x_ve, array_acc, marker='o', linestyle='dashed',label="nit")
